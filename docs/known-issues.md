...
| BUG-001 | Fixed awaiting retest | Reporter clarified that wild Mirelume appeared at 0 HP across encounters and appeared to heal at the party crystal; exact path is not reproduced by current source. | State ownership in the core capture loop requires target-device confirmation. | Retest 0.0.9 independent wild state, crystal isolation, and in-game reset. |
